...
const listInti = async (req, res) => {
    try {
        const {namaPuskesmas, alamatPuskesmas, noTelepon, maps, poli, denah, foto, id, idName} = req.query
        const { data, error } = await supabase
            .from('puskesmas')
            .select('id, namaPuskesmas, alamatPuskesmas, noTelepon, maps, poli, denah, foto')
            .eq('idName', idName)
        if (error) {
            return res.json(error)
        }
...
module.exports = {
    add, hapus, list, listInti, update
}